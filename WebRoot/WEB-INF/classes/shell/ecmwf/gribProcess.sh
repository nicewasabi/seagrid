...
for shortName in $(echo $allShortName | sed 's/_/\n/g'); do
	...
	if [ "$shrtName" == "10" -o "$shrtName" == "uv10" -o "$shrtName" == "100" -o "$shrtName" == "uv100" ]; then
		...
		python $dsOutputRootPath/bin/gribToMicaps11.py $grib1ParamExtractFile $dsmicapsRootPath/$shortName 95 142 45 -10 0.1 0.1
		
		# Get 10 meter wind North Indian Ocean and North Pacific Ocean data
		if [ "$shrtName" == "uv10" ]; then
			python $dsOutputRootPath/bin/gribToMicaps11.py $grib1ParamExtractFile $dsmicapsRootPath/uv10_NP 100 240 60 0 0.25 0.25
			python $dsOutputRootPath/bin/gribToMicaps11.py $grib1ParamExtractFile $dsmicapsRootPath/uv10_NI 40 100 30 -20 0.25 0.25
		fi
		...
		python $dsOutputRootPath/bin/gribToMicaps4_irregular.py $grib1ParamExtractFile $dsmicapsRootPath/$shortName 95 142 45 -10 0.1 0.1
	elif [ "$shrtName" == "r2" ]; then
		# Convert GRIB to MICAPS4
		python $dsOutputRootPath/bin/gribToMicaps4_r2.py $grib1ParamExtractFile $dsmicapsRootPath/$shortName 95 142 45 -10 0.1 0.1
	else
		# Convert GRIB to MICAPS4
		python $dsOutputRootPath/bin/gribToMicaps4.py $grib1ParamExtractFile $dsmicapsRootPath/$shortName 95 142 45 -10 0.1 0.1
	fi
