#!/bin/bash
# Filename: get_wamhres_00.sh
# Description: Process wamhres 00 data
# Date: 2017-11-21
# Author: songwj
# Version: v4
source /etc/profile

#pidList=$(ps aux | grep /space/data2/ecData/bin/get_wamhres_00.sh | grep -v grep | awk '{print $2}')
#[[ ! -z $pidList ]]
#if [ $? -eq 0 ]; then
#	echo "[$(date +'%F %T')] >>> Process [/space/data2/ecData/bin/get_wamhres_00.sh] is running, no need to process it again."
#	exit 0
#fi
...
for endStep in $(seq 0 3 72) $(seq 78 6 240); do
	...
	fName="$fName ${datetime}08.$endStep" # 18051508.024 18051508.027 ...
done
