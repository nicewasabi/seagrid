...
<html>
		    ...
		    <div id="secondContext">
				...
				<div id="mainContext" style="min-height: 40%;">
					...
					<ol class="breadcrumb" style="margin-top: 10px;margin-left: 10px;background-color: white;width: 40%;">
					  	...
					  	<li><a href="javascript:void(0)">海上10米8级大风概率/</a></li>
					  	<li class="active">北太平洋</li>
					</ol>
					<hr style="width:100%">
	...
	<script type="text/javascript">
		...
		function showImageProcess() {
			...
			// 产品类型
			var windType = $("input[name=windType]:checked").val();
			showImage(datetime, timelimit, windType);
		}
		
		// 显示图像
		function showImage(datetime, timelimit, windType) {
			$.post("${ctx}/np/validImage.do", {datetime:datetime, timelimit:timelimit, windType:windType}, function(data){
				// 清空内容
				$("#imgDiv").html();
				if(data == false){
					...
					var img = $("#img");
					var url = "${ctx}/np/showImage.do?datetime=" + datetime + "&timelimit=" + timelimit + "&windType=" + windType;
					img.attr("src", url);
				}
			});
		}
