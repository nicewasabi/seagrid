...
<script type="text/javascript"
	src="${ctx}/js/simpleFocus/simpleFocus.js"></script>
</head>
<body>
	<div style="width: 1350px; margin: 0 auto;">
		...
		<div id="secondContext">
			...
			<!-- 左侧菜单 -->
			<div id="leftContext"
				style="width: 266px; min-height: 500px; float: left; text-align: center;">
			...
			<div id="mainContext" style="min-height: 40%;">
				<!-- 导航栏 -->
				<ol class="breadcrumb"
					style="margin-top: 10px; margin-left: 10px; background-color: white; width: 40%;">
				...
				<div class="border_condition_main">
					起报日期： <input class="Wdate" id="datetime" type="text"
						style="width: 90px; max-width: 90px; height: 25px; margin: 0"
						onfocus="WdatePicker({skin:'twoer', dateFmt:'yyyyMMdd',autoPickDate: true});" />
					&nbsp;时间间隔(分钟)： <select id="step">
						<option value="05">05</option>
						<option value="15">15</option>
					</select> &nbsp;小时： <select id="hour">
						<c:forEach begin="0" end="23" step="1" varStatus="hourVar">
							<option><fmt:formatNumber pattern="00"
									value="${hourVar.index}"></fmt:formatNumber></option>
						</c:forEach>
					</select> &nbsp;分钟： <select id="selectMinutes">
					</select>
						...
						<tr>
							<td>
								<div id="leftPannel" class="pannel">
									<img src="${ctx}/images/simpleFocus/left_pannel.png"
										style="margin: 250px 17px;" />
								</div>
							</td>
							<td>
								<div id="imgDiv"
									style="width: 870px; height: 653px; text-align: center; margin: auto">
								</div>
							</td>
							<td>
								<div id="rightPannel" class="pannel">
									<img src="${ctx}/images/simpleFocus/right_pannel.png"
										style="margin: 250px 17px;" />
								</div>
							</td>
						</tr>
...
<script type="text/javascript">
	...
	function showImage(datetime, hour, step, minute) {
		$
				.post(
						"${ctx}/seaFogInversion/offshore/validImage.do",
						{
							datetime : datetime,
							hour : hour,
							step : step,
						...
						function(data) {
							...
							if (data == false) {
								$("#imgDiv")
										.html(
												"<img src='${ctx}/images/ui/nodata.jpg' style='width:100%;height:100%'/>");
							} else {
								$("#imgDiv")
										.html(
												"<img id='img' src='' style='width:100%;height:100%;border: 1px solid gray;' />");
								var img = $("#img");
								var url = "${ctx}/seaFogInversion/offshore/showImage.do?datetime="
										+ datetime
										+ "&hour="
										+ hour
										+ "&step="
	...
	function getMinutesList(datetime, hour, step) {
		$.post("${ctx}/seaFogInversion/offshore/getMinutesList.do", {
			datetime : datetime,
			hour : hour,
			step : step
		}, function(data) {
			if (data != null) {
				...
				$("#selectMinutes").prepend("<option value='0'>请选择</option>");
				 for (var i = 0; i < data.length; i++) {
					$("#selectMinutes").append(
							"<option value='"+data[i]+"'>" + data[i]
									+ "</option>");
				}
/* 				 $("#selectMinutes option:first").prop("selected", 'selected');
 */			}
		});
	}
</script>
