...
　　						 </select> 
	...
	<script type="text/javascript">
		...
		function showImage(datetime, hour, step,minute) {
			$.post("${ctx}/seaFogInversion/roofHeight/validImage.do", {datetime:datetime, hour:hour, step:step,minute:minute}, function(data){
				// 清空内容
				$("#imgDiv").html();
				if(data == false){
					...
					var img = $("#img");
					var url = "${ctx}/seaFogInversion/roofHeight/showImage.do?datetime=" + datetime + "&hour=" + hour + "&step=" + step+"&minute=" + minute ;
					img.attr("src", url);
				}
			});
		}
		...
		function getMinutesList(datetime, hour,step) {
			...
			$.post("${ctx}/seaFogInversion/roofHeight/getMinutesList.do", {datetime:datetime, hour:hour,step:step}, function(data){		
				if(data!=null){
					$("#selectMinutes").empty();
