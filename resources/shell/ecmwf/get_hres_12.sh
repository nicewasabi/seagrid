#!/bin/bash
# Filename: get_hres_12.sh
# Description: Process hres 12 data
# Date: 2017-11-21
# Author: songwj
# Version: v4
source /etc/profile

#pidList=$(ps aux | grep /space/data2/ecData/bin/get_hres_12.sh | grep -v grep | awk '{print $2}')
#[[ ! -z $pidList ]]
#if [ $? -eq 0 ]; then
#	echo "[$(date +'%F %T')] >>> Process [/space/data2/ecData/bin/get_hres_12.sh] is running, no need to process it again."
#	exit 0
#fi
...
for shortName in $(echo ${allShortName}_swh | sed 's/_/\n/g'); do
