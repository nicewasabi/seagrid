...
def getChineseByShortName(shortName, level):
	if shortName == 'swh':
		...
		return "相对湿度2m"
	elif shortName == 'swdir' and level == 1:
		return "涌浪方向层次1"
	elif shortName == 'swdir' and level == 2:
		return "涌浪方向层次2"
	elif shortName == 'swell' and level == 1:
		return "涌浪有效波高层次1"
	elif shortName == 'swell' and level == 2:
		return "涌浪有效波高层次2"
	else:
		return shortName
...
def getVarOutByShortName(shortName, level):
	if shortName == 'swh':
		...
		return [0.500, 0.000, 20.000, 0.000, 5.000]
	elif shortName == 'swdir':
		return [45.0, 0.0, 360.0, 1.0, 0.0]
	elif shortName == 'swell':
...
def gribToMicaps4():
	...
	while 1: 
		...
		forecastTime = grib_get(gid,'forecastTime') # 时效，如：240
		...
		# 写入描述
		fout.write("diamond 4 " + filename + "_" + getChineseByShortName(shortName, level) + "_0.5*0.5\n")
